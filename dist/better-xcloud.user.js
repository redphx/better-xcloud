...
class SettingsNavigationDialog extends NavigationDialog {
  ...
  TAB_GLOBAL_ITEMS = [{
    ...
    items: [
      ($parent) => {
        ...
        }, t("settings-reload-note")), topButtons.push(this.$noteGlobalReload), this.$btnSuggestion = createButton({
          label: ut("Suggest settings"),
          style: 32 | 64,
          onClick: this.renderSuggestions.bind(this)
  ...
  renderSuggestions(e) {
    const $btnSuggest = e.target.closest("button");
    let $content = $btnSuggest.nextElementSibling;
    if ($content) {
      $content.classList.toggle("bx-gone"), BxEvent.dispatch($content.querySelector("select"), "input");
      return;
    }
    const $suggestedSettings = CE("div", { class: "bx-suggest-wrapper" }), $select = CE("select", {}, CE("option", { value: "default" }, t("default")), CE("option", { value: "lowest" }, ut("Lowest quality")), CE("option", { value: "highest" }, ut("Highest quality")));
    $select.addEventListener("input", (e2) => {
      const profile = $select.value;
      if ($suggestedSettings.innerHTML = "", profile === "highest")
        $suggestedSettings.appendChild(CE("div", { class: "bx-suggest-note" }, ut("⚠️ Your device may not be powerful enough to use these settings")));
      const settings = this.suggestedSettings[profile];
      let prefKey;
      for (prefKey in settings) {
        const currentValue = getPref(prefKey), suggestedValue = settings[prefKey], currentValueText = STORAGE.Global.getValueText(prefKey, currentValue), isSameValue = currentValue === suggestedValue;
        let $child, $value;
        if (isSameValue)
        ...
        $suggestedSettings.appendChild($child);
      }
    }), BxEvent.dispatch($select, "input");
    const $btnApply = createButton({
      ...
      onClick: (e2) => {
        ...
        BxEvent.dispatch($select, "input"), console.log(this.settingElements);
      }
    });
    $content = CE("div", {
    ...
    }, BxSelectElement.wrap($select), $suggestedSettings, $btnApply), $btnSuggest?.insertAdjacentElement("afterend", $content);
  }
  ...
  renderSettingRow(settingTab, $tabContent, settingTabContent, setting) {
    ...
    if (settingTabContent.label && setting.pref) {
      if (this.suggestedSettingLabels[setting.pref] = settingTabContent.label, prefDefinition?.suggest)
        typeof prefDefinition.suggest.lowest !== "undefined" && (this.suggestedSettings.lowest[setting.pref] = prefDefinition.suggest.lowest), typeof prefDefinition.suggest.highest !== "undefined" && (this.suggestedSettings.highest[setting.pref] = prefDefinition.suggest.highest), this.suggestedSettings.default[setting.pref] = prefDefinition.default;
    }
    if (experimental)
      if (label = "🧪 " + label, !note)
        note = t("experimental");
