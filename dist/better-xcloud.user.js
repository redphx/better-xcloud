    ...
    const getOptionAtIndex = (index) => {
      ...
      if ($option) {
        if (content = $option.textContent || "", content && $option.parentElement.tagName === "OPTGROUP")
          content = $option.parentElement.label + " ❯ " + content;
      }
      if ($label.textContent = content, isMultiple)
  ...
  for (let groupLabel in SETTINGS_UI) {
    ...
    for (let settingId of settingItems) {
      ...
      if ($control instanceof HTMLSelectElement && getPref(PrefKey.UI_CONTROLLER_FRIENDLY))
        ...
        $elm = CE("div", { class: "bx-settings-row", "data-group": 0 }, $label, CE("div", { class: "bx-setting-control" }, $control));
